...
const ForgotModal = (props) => {
  ...
  const basForgotUrl =
    "https://identitytoolkit.googleapis.com/v1/accounts:sendOobCode?key=AIzaSyDHg3icjrRbCcrwXkQZPXfPCutfZg-Bm0Y";

  const handleClose = () => props.showModalHandler(false);

  const forgotPasswordHandler = async () => {
    const enteredEmail = enteredEmailRef.current.value;
    const siginupEmail = localStorage.getItem("email");
    ...
    if (enteredEmail === siginupEmail) {
      try {
        ...
        if (response.status === 200) {
          alert(
            "The password reset link has been successfully sent, please check your email!"
          );
        } else {
